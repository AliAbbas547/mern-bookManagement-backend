  ...
  try {
    ...
    if (!Validations.isValidString(title)) {
      return res
        ...
        .send({
          status: false,
          message: " provide valid title values,it can't be empty",
        });
    }
    ...
    if (!Validations.isValidStringName(title)) {
      return res
        .status(400)
        .send({ status: false, message: " provide valid title values" });
    }
    ...
    if (!excerpt) {
      return res
        .status(400)
        .send({ status: false, message: "please enter the  excerpt values" });
    }
    if (!Validations.isValidString(excerpt)) {
      return res
        ...
        .send({
          status: false,
          message: " provide valid excerpt values, it can't be empty",
        });
    }
    ...
    if (!Validations.isValidStringName(excerpt)) {
      return res
        ...
        .send({
          status: false,
          message:
            " provide valid excerpt string values, it must be a string values , cant't be a number",
        });
    }
    ...
    if (userIdValidation == null) {
      return res
        .status(404)
        .send({ status: false, message: "this user Id is not registered" });
    }

    if (!ISBN) {
      return res
        .status(400)
        .send({ status: false, message: "please enter the  ISBN" });
    }
    ...
    if (!Validations.isValidString(category)) {
      return res
        ...
        .send({
          status: false,
          message: " provide valid  category values, it cant't be empty",
        });
    }
    if (!Validations.isValidStringName(category)) {
      return res
        ...
        .send({
          status: false,
          message: " provide valid  category string  values",
        });
    }
    ...
    if (!Validations.isValidString(subcategory)) {
      return res
        ...
        .send({
          status: false,
          message: " provide valid subcategory values,it can't be empty",
        });
    }
    ...
    if (!Validations.isValidStringName(subcategory)) {
      return res
        ...
        .send({
          status: false,
          message: " provide valid  subcategory string values",
        });
    }
  ...
  try {
    ...
    if (BooksdatabyId == null) {
      return res
        ...
        .send({
          status: false,
          message: "this bookId is not registered with this userId",
        });
    }

    let Booksdata = await BooksModel.findOne({
      _id: bookId,
      isDeleted: false,
    }).lean();
    ...
    if (Booksdata == null) {
      return res
        ...
        .send({
          status: false,
          message: "your request is not correct,book is already deleted",
        });
    }

    let getBooksDataWithReviews = await ReviewModel.find({
      bookId: bookId,
      isDeleted: false,
  ...
  try {
    ...
    if (Object.keys(data).includes("excerpt")) {
      if (!Validations.isValidString(excerpt)) {
        return res
          .status(400)
          .send({ status: false, message: "provide valid excerpt values" });
      }
      if (!Validations.isValidStringName(excerpt)) {
        return res
          .status(400)
    ...
    //       .send({
    //         status: false,
    //         message: "provide valid date formate like(year,month,date)",
    //       });
    //   }
    // }
    ...
    if (BooksDataGettingWithId == null) {
      return res
        .status(404)
        .send({ status: false, message: " bookId is not correct " });
    }
    ...
    let updateBooksData = await BooksModel.findOneAndUpdate(
        {_id: bookId},
      { $set: data },
      { new: true }
    );
    ...
    res
      .status(200)
      .send({ status: true, message: "success", data: updateBooksData });
  } catch (error) {
    return res.status(500).send({ status: false, message: error.message });
  }
};
...
const deleteBooksData = async function (req, res) {
  try {
    ...
    let updateBooksData = await BooksModel.findOneAndUpdate(
      { _id: bookId },
      { $set: { isDeleted: true, deletedAt: t1 } },
      { new: true }
    );
