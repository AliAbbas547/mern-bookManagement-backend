...
const createReview = async function (req, res) {
  try {
    ...
    if (!bookId) {
      return res
        .status(400)
        .send({ status: false, message: "please enter the bookId" });
    }
    ...
    if (Object.keys(data).includes("reviewedBy")) {
      if (!Validations.isValidString(reviewedBy)) {
        return res
          .status(400)
          .send({ status: false, message: "reviewer's name must not empty" });
      }
      if (!Validations.isValidStringName(reviewedBy)) {
        return res
          ...
          .send({
            status: false,
            message: "reviewer's name must be a proper string value",
          });
      }
    }
    ...
    if (!reviewedAt) {
      return res
        .status(400)
        .send({ status: false, message: "please enter reviewed time" });
    }
    ...
    if (!Validations.isValidDate(reviewedAt)) {
      return res
        ...
        .send({
          status: false,
          message: "please provide the date like (year,month,day) ",
        });
    }
    ...
    if (!rating) {
      return res
        .status(400)
        .send({ status: false, message: "please enter the rating " });
    }
    ...
    if (!review) {
      return res
        .status(400)
        .send({ status: false, message: "please provide the review " });
    }
    if (!Validations.isValidString(review)) {
      return res
        .status(400)
        .send({ status: false, message: "review must not be empty" });
    }
    ...
    if (!Validations.isValidStringName(review)) {
      return res
        ...
        .send({
          status: false,
          message: "review must be a proper string value",
        });
    }
    ...
    if (booksIdValidation == null) {
      return res
        ...
        .send({
          status: false,
          message: "there is no any book  associated with this bookId",
        });
    }
    const booksValidation = await BooksModel.findOne({
      _id: bookId,
      isDeleted: false,
    });
    ...
    const newReviewData = await ReviewModel.find({
      bookId: bookId,
      isDeleted: false,
    }).select({ isDeleted: 0, createdAt: 0, updatedAt: 0, __v: 0 });

    const addReview = await BooksModel.findOneAndUpdate(
      { _id: bookId },
      {
        $inc: {
          reviews: 1,
    ...
    return res
      .status(200)
      .send({ status: true, message: "success", data: addReview });
  } catch (err) {
...
const  updateReviews = async function (req, res) {
  try {
    ...
    // if (Object.keys(data).includes("reviewedAt")) {
    //   return res
    //     .status(400)
    //     .send({
    //       status: false,
    //       message:
    //         " reviewedBy , rating and review is only present in the body in order to update",
    //     });
    // }
    ...
    if (!reviewedBy) {
      return res
        .status(400)
        .send({ status: false, message: "please enter reviewer's name" });
    }
    ...
    if (books == null) {
      return res
        ...
        .send({
          status: false,
          message: "this  bookId  is not present in database",
        });
    }
    const booksWithDeletedkey = await BooksModel.findOne({
      _id: bookId,
      isDeleted: false,
    }).lean();
    ...
    if (checkReviewData == null) {
      return res
        ...
        .send({
          status: false,
          message: "there is no  any review data associated with this reviewId",
        });
    }

    const updateNewReviews = await ReviewModel.findOneAndUpdate(
      { _id: reviewId, bookId: bookId },
      { $set: data },
      { new: true }
    ).select({ isDeleted: 0, createdAt: 0, updatedAt: 0, __v: 0 });

    if (updateNewReviews == null) {
      return res
        .status(404)
        .send({
          status: false,
          message: " this  bookId is not matching with reviewId",
        });
    }
    ...
    return res
      .status(200)
      .send({ status: true, message: "success", data: booksWithDeletedkey });
  } catch (err) {
    return res.status(500).send({ status: false, message: err.message });
  }
};
...
const deleteReviews = async function (req, res) {
  try {
    ...
    const deleteReviewsChecking = await ReviewModel.findOne({
      _id: reviewId,
      isDeleted: false,
    });
    ...
    const deleteReviews = await ReviewModel.findOneAndUpdate(
      { _id: reviewId, bookId: bookId },
      { $set: { isDeleted: true } },
      { new: true }
    );
    ...
    // return res.status(200).send({ status: true, data: createCollege })

    const checkNewReviewData = await ReviewModel.find({
      bookId: bookId,
    }).select({ isDeleted: 0, createdAt: 0, updatedAt: 0, __v: 0 });

    if (checkNewReviewData.length == 0) {
      return res.status(404).send({ status: false, data: "data not found" });
    }

    booksWithDeletedkey["reviewsData"] = checkNewReviewData;

    // return res.status(200).send({ status: true, data: booksWithDeletedkey })

    const booksWithUpdation = await BooksModel.findOneAndUpdate(
      { _id: bookId },
      {
        $inc: {
          reviews: -1,
    ...
    return res
      ...
      .send({
        status: true,
        message: " reviews decreases by 1",
        data: booksWithUpdation,
      });
  } catch (err) {
    return res.status(500).send({ status: false, message: err.message });
  }
